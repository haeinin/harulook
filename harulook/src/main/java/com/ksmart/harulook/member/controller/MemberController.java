...
public class MemberController {
	...
	public String userDeleteAdd(Model model,
		...
		log.debug("MemberController 회원탈퇴시 user목록삭제후 delete_user 입력");
	...
	public String logout(HttpSession session) {
		log.debug("MemberControll 로그아웃 aaaaaaa");
	...
	public String login(Model model,
		...
		log.debug("MemberController 로그인시 == " + id);
		MemberDto loginCheck = memberDao.selectlogin(id);
		log.debug("MemeberController 로그인시 체크되어 받은 아이디loginCheck== " + loginCheck);
		if(loginCheck == null){
			loginCheck = null;
			log.debug("아이디틀림");
			...
			if(pw.equals(loginCheck.getUserPw())) {
				...
				if(loginCheck.getUserLevel().equals("일반회원")){
					...
					log.debug("MemeberController 출석체크 중복검사== " + attenCheckSelect);
					int attenCheckSelectMonth = pointDao.selectAttenCheckMonth(loginCheck.getUserId());
					log.debug("MemeberController 출석체크 한달== " + attenCheckSelectMonth);
					//맞춤추천을 위해 세션에 정보 저장	
	...
	public String managerDetail(Model model,
		...
		log.debug("member_user_detail 일반회원정보보기할때 받아온 아이디 == " + userId);
		MemberDto businessDetail = memberDao.selectBusinessDetail(userId);
		model.addAttribute("businessDetail", businessDetail);
		log.debug("MemeberController 회원정보보기로 받아온 회원상세데이터들 model==" + model);
	...
	public String userDetail(Model model,
		...
		log.debug("MemeberController 회원정보보기로 받아온 회원의컬러==" + userColor);
		log.debug("MemeberController 회원정보보기로 받아온 회원의스타일==" + userStyle);
		log.debug("MemeberController 회원정보보기로 받아온 회원의컬러와스타일 model==" + model);
    ...
    public String allUserList(Model model,
		...
		log.debug(userId + " == 검색 아이디");
		
		if(userId == ""){	//검색 아이디가 공백일 경우 null로 바꾸어 전체 리스트 나열
			userId = null;
		}
		log.debug(levelList + " == 권한별로 리스트");
    	...
    	log.debug("MemeberController 셀렉트해서 받아온 일반 회원 리스트 model값 == "+model);
	...
	public String managerAdd(MemberDto memberDto) {
        log.debug("MemberController 회원가입시 받아오는 데이터" + memberDto);
	...
	public String businessUpdateAdd(MemberDto memberDto,
        ...
        log.debug("MemberController 회원정보수정하기" + memberDto);
	...
	public String managerAdd() {
		log.debug("member_manager_insert 관리자등록");
	...
	public String businessAdd() {
		log.debug("member_business_insert 사업자등록폼");
	...
	public String businessUpdate(Model model,
			HttpSession session,
			@RequestParam(value="userId", required=true) String userId) {
		log.debug("MemeberController 관리자 사업자수정폼 받아온 아이디 == " + userId);
	...
	public String userUpdate(Model model,
			HttpSession session,
			@RequestParam(value="userId", required=true) String userId) {
		log.debug("MemeberController 일반회원수정폼 받아온 아이디 == " + userId);
		if(userId == "") {	//비밀번호찾기에서 회원수정으로 넘어올때 세션에 있는 아이디를 넣기 위한 if문
			String sessionId = (String) session.getAttribute("id");
			userId = sessionId;
		}
		...
		MemberDto userDetail = memberDao.selectUserDetail(userId);
		model.addAttribute("userDetail", userDetail);
		log.debug("MemeberController 일반회원수정폼 받아온 회원상세데이터들 model==" + model);
		...
		List<String> userStyle = memberDao.selectUserStyle(userId);	
		model.addAttribute("userStyle", userStyle);
		log.debug("MemeberController 일반회원수정폼 받아온 회원의컬러와스타일 model==" + model);
		log.debug("member_user_update 일반회원수정홈화면");
	...
	public String userAdd() {
		log.debug("member_user_insert 일반회원가입홈화면");
		return "member/user/member_user_insert"; //일반회원가입폼화면
	}
	/*로그인화면폼*/
	@RequestMapping(value="/loginForm", method = RequestMethod.GET)
	public String login() {
		log.debug("login 로그인화면");
